function MeanAPmRNA(varargin)

%calculate the total mRNA production along AP Axis, modified from 
%'FitMeanAPAsymmetric.m'
...
for i=1:length(APbinID)
    if isempty(FitResults(i,1).Rate0)
        ...
        FitResults(i,1).TotalmRNA=[];
        FitResults(i,1).TotalmRNAFit=[];
end
%nc13
for i=1:length(APbinID)
...
%         FitResults(i,2).TotalmRNA=[];
%         FitResults(i,2).TotalmRNAFit=[];
...
for i=1:length(APbinID)
    ...
    FitResults(i,3).TotalmRNA=[];
    FitResults(i,3).TotalmRNAFit=[];    
end
        ...
        if (sum(NParticlesAP(FrameWindow,i)>=MinParticles)>=MinTimePoints)
            ...
            if CurrentNC~=14
                ...
                if ~isempty(TimeData(NanFilter))
                    ...
                    %calculate the total mRNA
                    if(sum(NanFilter)>1) %at least two point
                        FitResults(i,CurrentNC-11).TotalmRNA=trapz(TimeDataForFit(NanFilter),FluoDataForFit(NanFilter));
                    end
                        
%                     if(FitResults(i,CurrentNC-11).TimeEnd < Delay + FitResults(i,CurrentNC-11).TimeStart)
%                         FitResults(i,CurrentNC-11).TotalmRNAFit = 1/2*(FitResults(i,CurrentNC-11).TimeEnd...
%                             -FitResults(i,CurrentNC-11).TimeStart)^2*FitResults(i,CurrentNC-11).RateFit*(1+...
%                             FitResults(i,CurrentNC-11).RateFit/FitResults(i,CurrentNC-11).RateOffFit);
%                     else
%                         FitResults(i,CurrentNC-11).TotalmRNAFit = 1/2*(2*FitResults(i,CurrentNC-11).TimeEnd-...
%                             2*FitResults(i,CurrentNC-11).TimeStart+(FitResults(i,CurrentNC-11).RateFit/FitResults(i,CurrentNC-11).RateOffFit...
%                             -1)*Delay)*Delay*FitResults(i,CurrentNC-11).RateFit;
%                     end
                    ...
                    %calculate the total mRNA based on the fitted curve
                    FitResults(i,CurrentNC-11).TotalmRNAFit = trapz(TimeFit,FluoFit);
                    %Plot all the data
                    ...
                    legend(PlotHandle,['tON=',num2str(FitResults(i,CurrentNC-11).TimeStart),' \pm ',num2str(FitResults(i,CurrentNC-11).SDTimeStart)],...
                        ...
                        'Location','Best')
%                     ylim=get(PlotHandle,'ylim');
                    if(~isempty(FitResults(i,CurrentNC-11).TotalmRNA))
                        XLIM=get(gca,'xlim');
                        YLIM = ylim;
                        text(XLIM(1)+0.1*(XLIM(2)-XLIM(1)),YLIM(2)*0.2,['TotalmRNA=',num2str(FitResults(i,CurrentNC-11).TotalmRNA),' mRNAFit=',num2str(FitResults(i,CurrentNC-11).TotalmRNAFit)])
                    end
            ...
            elseif CurrentNC==14
                ...
                if ~isempty(TimeData(NanFilter))
                    ...
                    XLIM=get(gca,'xlim');
                    YLIM = ylim;
                    text(XLIM(1)+0.1*(XLIM(2)-XLIM(1)),YLIM(2)*0.2,['TotalmRNA=',num2str(FitResults(i,CurrentNC-11).TotalmRNA)]);
                end
            end
...
save([DropboxFolder,filesep,Prefix,filesep,'MeanmRNA.mat'],...
    'FitResults')
display('MeanmRNA.mat saved')        
